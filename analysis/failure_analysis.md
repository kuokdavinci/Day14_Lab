...
- **Hệ thống Ingestion:** Việc sử dụng `MarkdownLegalChunker` thay vì RecursiveChunker thông thường là chìa khóa để đạt **100% Hit Rate**. Nó giúp giữ nguyên ngữ cảnh của các "Điều" và "Khoản", tránh việc cắt nửa chừng làm Agent hiểu sai chính sách.
- **Hiệu năng:** Hệ thống hiện tại đang chạy ở mức an toàn để tránh Rate Limit. Trong tương lai, việc chuyển sang `acompletion` với một bộ Proxy xoay vòng API sẽ giúp đưa tốc độ xuống dưới 30 giây.
